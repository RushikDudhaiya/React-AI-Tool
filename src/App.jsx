  ...
  return (
    <div className={darkMode == 'dark' ? 'dark' : 'light'}>
      ...
      <div className='  grid grid-cols-5 h-screen text-center' >
        ...
        <select id='select' name='select' onChange={(event) => setDarkMode(event.target.value)} className='fixed text-zinc-400 text-sm bottom-0 p-5'>
        ...
        <div className=' col-span-4 p-10'>
          ...
          <div className=' dark:bg-zinc-800 bg-red-100 w-1/2 p-1 pr-5 dark:text-white text-zinc-800 m-auto rounded-4xl border border-zinc-700 flex h-16' >
            <input type="text" value={question} onKeyDown={isEnter}
              ...
              className=' w-full h-full p-3 outline-none' placeholder='Ask me everything' />

            <button onClick={() => askQuestion()} className=' dark:text-white text-lg'>Ask</button>
          </div>
        </div>
      </div >
    </div>
